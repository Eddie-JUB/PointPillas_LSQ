...
def main(args):
    ...
    # load the model
    data_dir = "/home/AI_system_design/PointPillars_2x2/"

    # with open(data_dir+'config_2x2.yaml') as yaml_file:
    # with open(data_dir+'config_4x4.yaml') as yaml_file:
    # with open(data_dir+'config_4x8.yaml') as yaml_file:
    with open(data_dir+'config_8x4.yaml') as yaml_file:
    # with open(data_dir+'config_8x8.yaml') as yaml_file:
    # with open(data_dir+'config_16x16.yaml') as yaml_file:
    ...
    quantization_args = munch.munchify(cfg)
    # print(quantization_args)


    if not args.no_cuda:

        pointpillars_org = PointPillars(nclasses=args.nclasses).cuda()
        modules_to_replace =  quan.find_modules_to_quantize(pointpillars_org, quantization_args.quan)
        model = quan.replace_module_by_names(pointpillars_org, modules_to_replace).cuda()

        # model = PointPillars(nclasses=args.nclasses).cuda()
        model.load_state_dict(torch.load(args.ckpt))
    else:

        pointpillars_org = PointPillars(nclasses=args.nclasses).cpu()
        modules_to_replace =  quan.find_modules_to_quantize(pointpillars_org, quantization_args.quan)
        model = quan.replace_module_by_names(pointpillars_org, modules_to_replace).cpu()
